      ...
      while ((minimizer_ptr = scanner.NextMinimizer()) != nullptr) {
        ...
        if (scanner.is_ambiguous()) {
          ...
          if (*minimizer_ptr != last_minimizer) {
            ...
            // I ADDED THIS LINE THAT PRINTS read ID, the taxon ID, and the minimizer value
            unsigned long long external_taxon_id = 0;
            if (taxon > 0 && taxon < taxonomy.node_count()) {
                external_taxon_id = taxonomy.nodes()[taxon].external_id;
            }
            printf("%s\t%llu\t%llu\n", dna.header.c_str(), external_taxon_id, (unsigned long long)*minimizer_ptr);
